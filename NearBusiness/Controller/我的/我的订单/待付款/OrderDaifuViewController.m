...
@interface OrderDaifuViewController ()<UITableViewDataSource,UITableViewDelegate,OrderDaiTableViewCellDelegate,OrderYi2TableViewCellDelegate>{
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (0 == indexPath.row) {
        ...
        static NSString *cellIdentifier = @"OrderDaiTableViewCell";
        OrderDaiTableViewCell* cell0 = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (!cell0) {
            cell0 = [[OrderDaiTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
        }
        cell0.OrderDaiTableViewCellDelegate = self;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (0 == indexPath.row) {
        ...
        return [OrderDaiTableViewCell hyb_heightForTableView:_tableView config:^(UITableViewCell *sourceCell) {
            OrderDaiTableViewCell *cell = (OrderDaiTableViewCell *)sourceCell;
            // 配置数据
            [cell configCellWithModel:model];
        }];
    }
