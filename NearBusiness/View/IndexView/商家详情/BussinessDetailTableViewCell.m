...
//
//  BussinessDetailTableViewCell.m
//  NearBusiness
//
...
@interface BussinessDetailTableViewCell()<CycleScrollViewDelegate>

@property (nonatomic, strong) CycleScrollView *cycleScrollView;

@end
@implementation BussinessDetailTableViewCell
...
- (void)makeUI{
    //网络加载 --- 创建带标题的图片轮播器
    self.cycleScrollView = [CycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 0, [MyController getScreenWidth], [MyController getScreenWidth] / 2) imageURLStringsGroup:nil]; // 模拟网络延时情景
...
- (void)cycleScrollView:(CycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index{
    ...
    [self.BussinessDetailTableViewCellDelegate didselectADPic:index];
}
