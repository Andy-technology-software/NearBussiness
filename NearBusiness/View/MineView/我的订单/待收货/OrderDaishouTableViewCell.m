...
@interface OrderDaishouTableViewCell()
...
@end
@implementation OrderDaishouTableViewCell
...
- (void)makeUI{
    ...
    self.payBtn = [MyController createButtonWithFrame:self.contentView.frame ImageName:nil Target:self Action:@selector(payBtnClick) Title:@"确认收货"];
    ...
    self.cancelBtn = [MyController createButtonWithFrame:self.contentView.frame ImageName:nil Target:self Action:@selector(cancleBtnClick) Title:@"查看物流"];
    [self.cancelBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.cancelBtn setBackgroundColor:[MyController colorWithHexString:@"19a0e5"]];
...
- (void)payBtnClick{
    [self.OrderDaishouTableViewCellDelegate didselectMakesure:self.sectionIndex];
}

- (void)cancleBtnClick{
    [self.OrderDaishouTableViewCellDelegate didselectCheckWL:self.sectionIndex];
}
